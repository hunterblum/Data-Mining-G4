...
Cat_eda <- function(x, y) {
  p1 <- ggplot(Stroke, aes(x={{x}})) + geom_bar(aes(fill=stroke)) + ggtitle(paste0("Stroke with Respect to ", y)) + xlab(y)         + ylab("Count")

  p2 <- ggplot(Stroke, aes(x={{x}})) + geom_bar(aes(fill=stroke), position = "fill") + ggtitle(paste0("Stroke with Respect to ",y, " (Normalized)")) + xlab(y) + ylab("Count")
...
#Numeric
Num_eda <- function(x, y, z) {
  p1 <- ggplot(Stroke, aes(x={{x}})) +        geom_histogram(aes(fill=stroke), color = "black", binwidth = z) +
    ggtitle(paste0("Stroke with Respect to ", y)) + xlab(y) + ylab("Count")

 p2 <- ggplot(Stroke, aes(x={{x}})) +
   geom_histogram(aes(fill=stroke), color = "black", position =      "fill", binwidth = z) +
   ggtitle(paste0("Stroke with Respect to ", y)) + xlab(y) +         ylab("Count")
